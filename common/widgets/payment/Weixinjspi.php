...
class Weixinjspi extends Payment
{
    ...
    {
        $paras = [
            'body'              =>$order['goods_desc'],//设置商品或支付单简要描述
            'attach'            =>'PC',//设置附加数据，在查询API和支付通知中原样返回，该字段主要用于商户携带订单的自定义数据
            'out_trade_no'      =>$this->order_pre.$order['order_sn'],//订单号
            'total_fee'         =>(int)($order['total_fee'] * 100),//订单总金额
            'detail'            =>$order['body'],
        ...
        WeixinJSBridge.invoke(
            ...
            function(res){
                 ...
                 if(res.err_msg == \"get_brand_wcpay_request:ok\")
                 {
                     $.ajax({
                            type:\"get\",
                            url:\"/clue/pay_status?order_sn=".$order_sn."\",
                            datatype:\"json\",
                            data:{},
