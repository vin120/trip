	...
	<div class="searchResult">
		...
		<table id="service_table">
			...
			<tbody>
				...
				<tr>
					<td><input type="checkbox" name="ids[]" value="<?php echo $row['service_id']?>"></input></td>
					<td><?php echo ($k+1)?></td>
					...
					<td class="op_btn">
						<a href="<?php echo Url::toRoute(['service/edit','id'=>$row['service_id']]);?>"><img src="<?php echo $baseUrl; ?>images/write.png"></a>
						<a id="<?php echo $row['service_id']?>" class="delete"><img src="<?php echo $baseUrl; ?>images/delete.png"></a>
					</td>
				</tr>
				<?php }?>
			</tbody>
...
window.onload = function(){
	...
	?>
		$('#page_div').jqPaginator({
		    ...
		    onPageChange: function (num, type) {
		    	...
		    	$.ajax({
	                url:"<?php echo Url::toRoute(['getservicepage']);?>",
	                type:'get',
	                data:'pag='+num,
	             	dataType:'json',
	            	success:function(data){
	            		...
	            		if(data != 0){
	    	                $.each(data,function(key){
	    	                	...
	    	                	str += '<tr>';
								str += '<td><input type="checkbox" name="ids[]" value="'+data[key]['service_id']+'"></input></td>';
								str += '<td>'+(key+1)+'</td>';
								str += '<td>'+data[key]['name']+'</td>';
								str += '<td>';
								var attr = '';
								$.each(data[key]['child'],function(v){
									attr +=  data[key]['child'][v]['name'];
								});
								...
								str += '<td>'+state+'</td>';
								str += '<td class="op_btn">';
								str += '<a href="<?php echo Url::toRoute(['service/edit']);?>?id='+data[key]['service_id']+'"><img src="<?php echo $baseUrl; ?>images/write.png"></a>';
								str += '<a id="'+data[key]['service_id']+'" class="delete"><img src="<?php echo $baseUrl; ?>images/delete.png"></a>';
								str += '</td>';
								str += '</tr>';


	                          });
	    	                $("table#service_table > tbody").html(str);
	    	            }
	            	}
	            });
	...
	$(document).on('click',"#promptBox > .btn .confirm_but_more",function(){
		$("form#service_index_form").submit();
	});
