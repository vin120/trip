...
<div class="r content" id="refundReason_content">
	<div class="topNav"><?php echo yii::t('app','咨询中心')?>&nbsp;&gt;&gt;&nbsp;<a href="<?php echo Url::toRoute(['index']);?>"><?php echo yii::t('app','信息分类')?></a></div>
	...
	?>
		<table id='message_type_table'>
            <input type="hidden" id="message_type_page" value="<?php echo $message_type_page;?>" />
			<thead>
				<tr>
					<th><input type="checkbox"></input></th>
					<th><?php echo yii::t('app','序号')?></th>
					<th><?php echo yii::t('app','图片')?></th>
					<th><?php echo yii::t('app','消息类型')?></th>
        ...
        <!-- 分页 -->
       <div class="center" id="message_type_page_div"> </div>
	</div>
</div>
<!-- content end -->
...
window.onload = function(){
    ...
    <?php $message_type_total = (int)ceil($count/10);
	   if($message_type_total >1){
	?>
		$('#message_type_page_div').jqPaginator({
		    totalPages: <?php echo $message_type_total;?>,
		    visiblePages: 10,
		    currentPage: 1,
		    ...
		    onPageChange: function (num, type) {
		    	var this_page = $("input#message_type_page").val();
		    	if(this_page==num){$("input#message_type_page").val('fail');return false;}

		    	$.ajax({
	                url:"<?php echo Url::toRoute(['getmessagetypepage']);?>",
	                type:'get',
	                data:'pag='+num,
	             	dataType:'json',
	            	success:function(data){
	            		...
	            		if(data != 0){
	    	                ...
	    	                $("table#message_type_table > tbody").html(str);
	    	            }
	            	}
	            });
