		...
		_attachments: function(input, inst) {
			...
			if (showOn === "div") {
				...
				inst.trigger = $(this._get(inst, "buttonImageOnly") ? $("<div/>").addClass(this._triggerClass).attr({
					...
					alt: buttonText,
					title: buttonText
				}) : $("<button type='button'></button>").addClass(this._triggerClass).html(!buttonImage ? buttonText : $("<div/>").attr({
					src: buttonImage,
					alt: buttonText,
					title: buttonText
				})));
...
var orderDays = $(".save-data-info").attr("data-info");
if (orderDays != undefined && orderDays != "") {
	orderDays = JSON.parse(orderDays);
}
var senseluxuryFed = {
	...
	loadListFun: function() {
		...
		_this.commonFun.fixedFun('.D1Search', 'fiexd');
	},
...
$('#f-price-checkbox-ddxp-icon').click(function() {
	if (!$(this).hasClass('cur')) {
		$("input[name='flight_info[]']").removeAttr('readonly').css('background-color', 'white');
		$('#reach').datepicker();
		$('#leave').datepicker();
	} else {
		$("input[name='flight_info[]']").attr('readonly', 'readonly').css('background-color', '#EEE');
	}
});
...
senseluxuryFed.detailFunV1 = function() {
	...
	$infoText.on('click', '.more', function() {
		...
		if ($this.hasClass('open')) {
			...
			$this.addClass('open').html('【' + MESSAGE.show_less_rates + ' - 】');
...
senseluxuryFed.detailFunV1.dateFun = function() {
	...
	if (getCookie("c_end_time")) {
		...
		var _start = $('#book_start').val();
		var _end = $("#book_end").val();
		isOrderDate._True = 0;
		isOrderDate._DateList = orderDays;
		if (isOrderDate.getDateList(_start, _end)) {
...
senseluxuryFed.commonFunV1.datepickerFun = function(id) {
	$(id).datepicker({
		...
		beforeshowday: senseluxuryFed.setDisableDays,
		minDate: 0,
...
senseluxuryFed.commonFun = function() {
	...
	$(function() {
		...
		_this.Autocomplete('#AutoSearch');
		_this.commonFun.ShouCan();
		_this.commonFun.ShouCanFunData(null, '3');
		_this.commonFun.ShouCanFun($.cookie("willdata"));
		_this.commonFun.closeShouCan();
		_this.commonFun.zixunSubmit();
		_this.commonFun.sideBarFun();
...
senseluxuryFed.commonFun.fixedFun = function(obj, className) {
	try {
		var dom = $(obj),
			...
			isIE6 = window.XMLHttpRequest ? false : true,
			pageName = pageID,
			leftHeight = dom.height();
		if (obj == '.tm_left') h = h - 150;
	} catch (e) {}
	if (!dom.length) return false;
	if (!isIE6) {
		$(window).on('scroll', function() {
			var windowTop = $(window).scrollTop();
			leftHeight = dom.height();
			var rightHeight = $('.TM_mbox').find('.tm_right').height();
			if (windowTop >= h) {
				dom.addClass(className);
				if (pageName == 'detail' || pageName == 'bankdetail') {
					if (windowTop > ($('.like-wp').offset().top - leftHeight - $("#fixed-menu").height() - 13)) {
						var top = $('.like-wp').offset().top - leftHeight;
						dom.css({
							position: "absolute",
							top: top
						});
					...
					if (windowTop > ($('.Cfoot').offset().top - leftHeight - $(".D1Search.fiexd").height())) {
						var top = $('.Cfoot').offset().top - leftHeight;
						dom.css({
							position: "absolute",
							top: top
						});
					} else if ((rightHeight - leftHeight > 20)) {
						dom.css({
							position: "",
							top: ""
						});
...
senseluxuryFed.indexFun.ajaxLoadXilie = function() {
	...
	function addHtmlFun(data) {
		...
		for (var i = 0; i < data.length; i++) {
			var price = (data[i]['price'] > 100) ? '￥' + data[i]['price'] + '/晚起' : '<span style="font-size: small">详细价格请咨询客服</span>';
			html += '<li>';
			...
			html += '<div class="Lc999 Lpl10 Lpr10">' + data[i]['parent_name'] + '</div>';
			html += '<div class="divyy"><span>' + data[i]['memo'] + '</span></div>';
			html += '</li>';
		}
		html += '</ul>';
		$('#con_tab_1').html(html);
...
senseluxuryFed.userdata = function() {
	var obj = $('.pages-selection'),
		_this_url = obj.attr('data-url'),
		_this_classname = $('.' + obj.attr('data-name')),
		domain = '',
		_page = '',
		PageAll = '',
		PageNow = '';
	...
	function htmlPages() {
		var htmlPage = '',
			...
			pAll = '';
		pAll = zdyNum;
		PageAll = parseInt(obj.attr('data-page-all'));
		PageNow = parseInt(obj.attr('data-page-now'));
		if (PageAll > pAll) {
			var n = (zdyNum - 1) / 2;
			if (n < PageNow) {
				goNum = PageNow - n;
		...
		htmlPage += '<a href="javascript:;" data-page="p" class="prev">' + MESSAGE.pre + '</a>';
		htmlPage += htmlt;
		for (var pageI = goNum; pageI <= pAll; pageI++) {
			var cur = (pageI == PageNow) ? ' class="cur"' : '';
			htmlPage += '<a href="javascript:;" data-page="' + pageI + '"' + cur + '>' + pageI + '</a>'
		}
		htmlPage += htmld;
		htmlPage += '<a href="javascript:;" data-page="n" class="next">' + MESSAGE.nex + '</a>';
		obj.find('.pages').html(htmlPage);
	}

	function datalist() {
		$.ajax({
			url: domain + _this_url,
			type: "GET",
			data: {
				page: _page,
				state: _state,
			...
			success: function(data) {
				...
				_this_classname.html(data);
			}
		});
	}
	obj.find('.pages').on('click', 'a', function() {
		_page = $(this).attr('data-page');
		_state = $('#hid_state').val();
		if ($(this).attr('data-page') == 'p') {
			var p = PageNow - 1;
			_page = p > 0 ? p : 1;
		} else if ($(this).attr('data-page') == 'n') {
			var p = PageNow + 1;
			_page = p > PageAll ? PageAll : p;
		}
		obj.attr('data-page-now', _page);
		htmlPages();
		datalist();
	})
};
...
senseluxuryFed.detailFun = function() {
	...
	_this.detailFunV1.picSlide('#dt_slides', '.tabLabel');
	deFun.dateFun();
	deFun.fixedMenuFun();
	...
	if (villa != '') {
		$.ajax({
			url: "/web/detail/getHouseDatas/villa/" + villa + "?is_all=1",
			dataType: "json",
			success: function(data) {
				orderDays = data.serialize;
				orderDays_extra = data.unserialize;
			}
		});
	}
	var $queryPrice = $('.query-price');
...
senseluxuryFed.detailFun.yudingPop = function() {
	...
	$("#book_start").on('change', function() {
		alert("dianji");
		$("#start").val($(this).val());
		$("#start1").val($("#book_end").val());
		if (datetime_to_unix_d($("#start").val()) >= datetime_to_unix_d($("#start1").val())) {
			alert('离开日期应该在起住日期之后！')
			return false;
		}
		$("#start").trigger('change');
		$('.f14 i').html((datetime_to_unix_d($("#start1").val()) - datetime_to_unix_d($("#start").val())));
		setCookie("c_start_time", $(this).val(), '1d');
		$("#start").val(getCookie("c_start_time"));
	});
	if (getCookie("c_end_time")) {
	...
	$('#order-btn').click(function() {
		if ($("#start").val() && $("#start1").val()) {
			...
			$('.Cmask').hide();
			$orderCont.hide();
			$("#start").next().trigger('click');
		} else if ($("#start").val() && !$("#start1").val()) {
			$('.Cmask').hide();
			$orderCont.hide();
			$("#start1").next().trigger('click');
		}
		if (isOrderDate._True) {
			alert(MESSAGE.house_status_busy);
			return false;
		} else {
			$("#price_cal_notice").html("");
		}
		$("#room-type").attr('data-val', $("#bedroom_show").attr('data-val'));
		$("#room-type").attr('data-flag', $("#bedroom_show").attr('data-flag'));
		$("#room-type").attr('data-standard', $("#bedroom_show").attr('data-standard'));
		$("#room-type").html($("#bedroom_show").html());
		$("#span-val").attr('data-val', parseInt($("#bedroom_show").attr('data-val')) * 2);
		$("#span-val").html(parseInt($("#bedroom_show").attr('data-val')) * 2);
		$('.f14 i').html((datetime_to_unix_d($("#start1").val()) - datetime_to_unix_d($("#start").val())));
		$("#book_bedroom").val($("#bedroom_show").attr('data-val'));
		$("#book_flag").val($("#bedroom_show").attr('data-flag'));
		$("#book_standard").val($("#bedroom_show").attr('data-standard'));
		$("#book_adult").val(parseInt($("#bedroom_show").attr('data-val')) * 2);
	});
	...
	$('.orderDialog form').validate({
		...
		submitHandler: function(form) {
			if (!$('#book_start').val()) {
				$('#book_start').parent().next().html('必填字段');
				return false;
			}
			if (!$('#book_end').val()) {
				$('#book_end').parent().next().html('必填字段');
				return false;
			}
			if (isOrderDate._True) {
				alert(MESSAGE.house_status_busy);
...
senseluxuryFed.setDisableDays = function(date) {
	var isOrdered = false;
	var scheduleStatus = "";
	var tmpData;
	if (orderDays == "" || orderDays == undefined) {
		tmpData = 0;
	} else {
		tmpData = orderDays;
	}
	if ($(this).attr("id") == "start1" || $(this).attr("id") == "book_end" || $(this).attr("id") == "inquireus_end") {
		if (orderDays_extra == "" || orderDays_extra == undefined) {
			tmpData = 0;
		} else {
			tmpData = orderDays_extra;
		}
	}
	if (tmpData != 0) {
		for (i = 0; i < tmpData.length; i++) {
			if (date.getMonth() == tmpData[i][0] - 1 && date.getDate() == tmpData[i][1] && date.getFullYear() == tmpData[i][2]) {
				isOrdered = true;
				break;
			}
		}
...
var isOrderDate = {
	...
	getTrue: function(orderDateList, Date_str) {
		for (i in orderDateList) {
			if (orderDateList[i].toString() == Date_str.toString()) {
				return 1;
			}
		}
	},
	getDateList: function(start, end) {
		...
		while (!(start.getFullYear() == end.getFullYear() && start.getMonth() == end.getMonth() && start.getDate() == end.getDate())) {
			if (this.getTrue(this._DateList, [(start.getMonth() + 1), start.getDate(), start.getFullYear()])) {
				this._True = 1;
				break;
			}
...
senseluxuryFed.commonFun.datepickerFun = function(id) {
	$(id).datepicker({
		showOn: "button",
		...
		onClose: function(selectedDate) {
			...
			if (id == "#start1") {
				...
				var _end = $(id).val();
				isOrderDate._True = 0;
				isOrderDate._DateList = orderDays_extra;
				if (isOrderDate.getDateList(_start, _end)) {
					$("#price_cal_notice").html("提示：" + MESSAGE.house_status_busy);
				} else {
					$("#price_cal_notice").html("");
...
senseluxuryFed.commonFun.zixunSubmit = function() {
	var comFun = senseluxuryFed.commonFun;
	comFun.datepickerFun('#inquire_start');
	comFun.datepickerFun('#inquire_end');
	if (getCookie("c_start_time")) {
...
senseluxuryFed.setCookie = function($this, idVal, pageID) {
	...
	if ($self.hasClass('curr')) {
		...
		_this.commonFun.ShouCanFunData(id, '2');
...
senseluxuryFed.commonFun.ShouCanFun = function(data) {
	...
	if (arrVal.length > 0) {
		...
		senseluxuryFed.commonFun.ShouCanFunData(data, '1');
...
senseluxuryFed.commonFun.ShouCanFunData = function(data, dowith) {
	var _this = senseluxuryFed;
	if (null == data) return;
	if (dowith == '1') {
		var arrVal = data || [];
		...
		$.ajax({
			...
			success: function(data) {
				if (data['code'] == 200) {
					var arrVal = data.val;
					if (arrVal != null) {
						$headerCont = $('.header'), $navCont = $headerCont.find('.nav'), $navListCont = $navCont.find('.navlink5'), shouCanHtmlList = '', pageIdVal = pageID;
						var _this = senseluxuryFed;
						var existVal = $.cookie('willdata') || [];
						arrVal = arrVal.concat(existVal);
						var n = {},
							r = [];
						for (i = 0; i < arrVal.length; i++) {
							if (!n[arrVal[i]['p_id']]) {
								n[arrVal[i]['p_id']] = true;
								r.push(arrVal[i]);
							}
						}
						arrVal = r;
						if (arrVal.length > 0) {
							...
							$('#shoucanList').html($(shouCanHtmlList));
							$.cookie('willdata', null);
							$.cookie('willdata', arrVal, {
								expires: 365,
								path: '/'
							});
						} else {
...
senseluxuryFed.getSearchList = function(data, pageName) {
	...
	for (var i = 0; i < aData.length; i++) {
		aHtml += '<li>';
		aHtml += '<a class="tag t_l" href="' + aData[i]['url'] + '#bspj-tit" title="' + MESSAGE.reviews + '（' + aData[i]['comments'] + '）">' + '<i class="icon iconfont newicon">&#xe603;</i>&nbsp;' + aData[i]['comments'] + '<div class = "comment_tip">评论</div></a>'
		aHtml += '<a class="tag t_r" href="javascript:void(0);" title="' + MESSAGE.stayed + aData[i]['been'] + '">' + '<i class="icon iconfont newicon">&#xe602;</i>&nbsp;' + aData[i]['been'] + '<div class = "been_tip">去过</div></a>';
		aHtml += '<a target="_blank" href="' + aData[i]['url'] + '" class="pic">' + '<img title="第六感 别墅度假 伊曼别墅The Iman Villa 别墅预订，主题度假在线预订" alt="度假别墅-' + aData[i]['title'] + '" class="lazy" data-original="' + aData[i]['imageUrl'] + '" src="resource/img/default-info.png"/> </a>';
		...
		aHtml += '<div class="serve-type"><p class="ms">' + aData[i]['subject'].join('<span>|</span>') + ' </p>';
		aHtml += '<p class="fw">';
		if (aData[i]['server'].replace("&nbsp;", "") != "") {
			aHtml += MESSAGE.service + '：' + aData[i]['server'];
		}
		aHtml += '</p></div>';
		aHtml += '<div class="inner">';
		aHtml += '<a target="_blank" href="' + aData[i]['url'] + '" class="xq-btn">' + MESSAGE.book + '</a>';
		aHtml += '<a href="javascript:void(0);" data-id="' + aData[i]['id'] + '" class="icon favorite' + rCookie(aData[i]['id']) + '"></a>';
		if (aData[i]['price'] > 100) {
			if (MESSAGE.night == "NT") {
				MESSAGE.n = "night";
				MESSAGE.p = "person";
			} else {
				MESSAGE.n = MESSAGE.night;
				MESSAGE.p = "";
			}
			var m = "";
			if (aData[i]['house_state'] != 0) {
				m = ' <img style="width: 9px;position: absolute;margin-left: -9px;" src="resource/img/icon_xianshi.png">'
			}
			aHtml += '<p class="price">' + m + '￥' + aData[i]['price'] + '/<small>' + MESSAGE.night + '</small><span>&nbsp;&nbsp;&nbsp;' + MESSAGE.p + '&nbsp;&nbsp;￥' + aData[i]['perCapita'] + '/&nbsp;' + MESSAGE.n + '</span></p>';
		} else {
			aHtml += MESSAGE.contact;
		}
		aHtml += '</div></div>';
		if (aData[i]['discount_ratio'] < 1 || aData[i]['is_discount'] == 1) {
			aHtml += '<span class="xsyh-icon new-s-off">';
			if (aData[i]['is_discount'] == 1) {
				var di = aData[i]['discount_info'].split("#");
				aHtml += "<p>" + di[0] + "</p>";
			} else {
				var discout = parseInt((1 - aData[i]['discount_ratio']).toFixed(2) * 100);
				aHtml += "<p>" + discout + "%OFF</p>";
			}
			aHtml += '<p>' + MESSAGE.promotion + '</p>';
			aHtml += '</span>';
		}
		if (aData[i]["is_panorama"]) {
			aHtml += ' <a href="javascript:void(0)"><div class="panorama"><span>全景</span></div></a>'
		}
		aHtml += '</li>';
	}
	aHtml += '</ul>';
	...
	function rCookie(id) {
		var willdata = $.cookie('willdata') || [];
		for (var di = 0; di < willdata.length; di++) {
			if (willdata[di].p_id == id) {
				return " curr";
			}
		}
		return "";
	...
	var start = (data['page'] - 1) * 15 + 1;
	var end = (data['page'] - 1) * 15 + 15 > data['total'] ? data['total'] : (data['page'] - 1) * 15 + 15;
	$("#total").html(start + "-" + end + ", " + data['total']);
	$('#sum-price').html(data['total']);
...
senseluxuryFed.pageClickFun = function(pageNum) {
	var href = window.location.href,
		...
		priceTip = $('.search-result-hd').find('.price').attr('data-tip') || '',
		domain = $('#domain').val();
	if ($pageName === "article" || $pageName === "place") $TM_mbox_top = $('.tips_list').offset().top;
	else $('.TM_mbox').offset().top;
	$destina_as.each(function(i) {
		$destina_arr.push($(this).attr('c_id'));
	});
	$destina_val = $destina_arr.join('|');
	...
	case 'subject':
		$.ajax({
			url: domain + '/subject_list/' + $subId + '?page=' + pageNum + '&callback=jsonp',
			type: "POST",
			data: {
				destination: $destina_val,
				order: priceTip,
	...
	case 'destinations':
		$.ajax({
			url: domain + '/destinations_list/' + $subId + '?page=' + pageNum + '&callback=jsonp',
			type: "POST",
			data: {
				order: priceTip,
				styles: $style_val,
	...
	case 'search':
		$.ajax({
			url: domain + '/search_list?q=' + $subId + '&page=' + pageNum + '&callback=jsonp',
			type: "POST",
			data: {
				min_bedroom: $minRoom,
				max_bedroom: $maxRoom,
	...
	case 'discount':
		$.ajax({
			url: domain + '/discount_list/' + $subId + '?page=' + pageNum + '&callback=jsonp',
			type: "POST",
			data: {},
			dataType: "jsonp",
			jsonpCallback: 'jsonp',
	...
	case 'article':
		$.ajax({
			url: domain + '/article_list/' + '?page=' + pageNum + '&callback=jsonp',
			type: "POST",
			data: {},
			dataType: "jsonp",
			jsonpCallback: 'jsonp',
			success: function(data) {
				if (data['code'] == 200) {
					$('html,body').animate({
						...
						var page = data["val"];
						$(".tips_list_main").remove();
						$(".module_title").after(page["data"]);
						senseluxuryFed.getPageFun(page["page"], page["pageTotal"]);
						window.picbander();
	...
	case 'place':
		...
		$.ajax({
			url: domain + '/article_list_one/' + '?page=' + pageNum + '&callback=jsonp' + '&des_id=' + des_id,
			type: "POST",
			data: {},
			dataType: "jsonp",
			jsonpCallback: 'jsonp',
			beforeSend: function() {
				$(".vaction-img").html("<img style='height:65px;width:58px;margin-left:100px;margin-top:60px;' src='resource/img/loading.gif' alt=''>");
			},
			success: function(data) {
				if (data['code'] == 200) {
					$('html,body').animate({
						...
						$(".module_title").after(page["data"]);
						senseluxuryFed.getPageFun(page["page"], page["pageTotal"]);
						$(".vaction-detail").each(function(i, item) {
							var a = $(item).attr("data-star-num") - 1;
							setStar(a, $(item).find(".five-star li"));
						});
					});
				};
			}
	...
	case 'view':
		var artId = $('#artId').val();
		$viewTop = $('.user-input-btn').offset().top;
		$.ajax({
			url: domain + '/review/' + '?page=' + pageNum + '&id=' + artId + '&type=' + 'view',
			type: "POST",
			data: {},
			dataType: "jsonp",
			jsonpCallback: 'jsonp',
			success: function(data) {
				if (data['code'] == 200) {
					$('html,body').animate({
						...
						var page = data["val"];
						$(".user-comment-show").remove();
						$(".user-input").after(page["data"]);
						senseluxuryFed.getPageFun(page["page"], page["pageTotal"]);
						getStart();
					});
				};
	...
	case 'shop':
		...
		$viewTop = $('.user-input-btn').offset().top;
		$.ajax({
			url: domain + '/review/' + '?page=' + pageNum + '&id=' + artId + '&type=' + 'shop',
			type: "POST",
			data: {},
			dataType: "jsonp",
			jsonpCallback: 'jsonp',
	...
	case 'food':
		...
		$viewTop = $('.user-input-btn').offset().top;
		$.ajax({
			url: domain + '/review/' + '?page=' + pageNum + '&id=' + artId + '&type=' + 'food',
			type: "POST",
			data: {},
			dataType: "jsonp",
			jsonpCallback: 'jsonp',
	...
	case 'entertainment':
		...
		$viewTop = $('.user-input-btn').offset().top;
		$.ajax({
			url: domain + '/review/' + '?page=' + pageNum + '&id=' + artId + '&type=' + 'entertainment',
			type: "POST",
			data: {},
			dataType: "jsonp",
			jsonpCallback: 'jsonp',
...
function setStar(index, domname) {
	domname.removeClass("solid-five-star")
	for (var i = 0; i <= index; i++) {
		domname.eq(i).addClass("solid-five-star");
	}
}

function getStart() {
	$(".user-comment-show .comment-show-item").each(function(i, item) {
		var a = $(item).attr("data-star-num") - 1;
		setStar(a, $(item).find(".five-star li"));
	});
	$(".container-item .new-comment-item").each(function(i, item) {
		var a = $(item).attr("data-star-num") - 1;
		setStar(a, $(item).find(".five-star li"));
	});
	$(".user-comment-show .comment-show-item").each(function(i, item) {
...
senseluxuryFed.detailFun.fixedMenuFun = function() {
	...
	$('.floor-index').each(function(i) {
		arrTop.push($(this).offset().top - 140);
	});
	arrTop[0] = $menuCont.offset().top - 50;
	$(window).scroll(function() {
		var winTop = $(window).scrollTop();
		if (winTop > menuTop) {
			$fixMenu.show();
...
senseluxuryFed.Autocomplete = function(obj) {
	...
	if (getCookie("c_guest_amount") && getCookie("c_guest_amount") != "undefined") {
		$('.D1main>ul>.li4>input').val(getCookie("c_guest_amount"))
		$('.D1main>ul>.li4>input').attr("data-valname", getCookie("c_guest_amount"));
	}
	...
	$('.search-btn').on('click', '.btn', function() {
		var keyVal = $inputId.val(),
			guestAmount = $('.D1main>ul>.li4>input').val(),
		...
		if (guestAmount == $('.D1main>ul>.li4>input').attr('data-valname')) {
			guestAmount = false;
		} else {
...
senseluxuryFed.detailFun.getCommentList = function(data) {
	...
	for (var i in aData) {
		aHtml += '<li> ' + '<div class="com-top">    ' + '<i class="rx-icon">      ' + '   <img src="' + aData[i]["headerimg"] + '" class="avatar"> ' + '</i> ' + '<span class="hes">' + aData[i]["userName"] + '</span> ' + '<span class="hus">' + aData[i]["addTime"] + '</span> ' + '</div>    ' + '<div class="yhlf-bg">       ' + ' <div class="yhrt-bg"><p>' + aData[i]["info"] + '</p></div>' + '</div>' + '<div class="comment">       ' + '<div class="comment-big-img">' + '<span class="arrow-bar arrow-l"></span>' + '<span class="arrow-bar arrow-r"></span>' + '<img src="" title="缩小"/>' + '</div> '
		for (var d in aData[i]["pic"]) {
			var uri = 'http://statics.hivilla.com/' + aData[i]["pic"][d]['small_url'];
			aHtml += '<img class="upImg"  style="background: url(' + uri + ') center no-repeat;background-size: cover;" date-big-url="' + uri + '">';
			if (parseInt(d) > 7) {
				aHtml += ' <div class="more-upImg">更多</div>' + '<div class="app-qrcode-detail color-444 font-12 c-f"><div>扫描二维码下载第六感APP，</div> 晒单或发表精彩评论获取现金券</div>';
			}
		}
		aHtml += '</div>'
		if (aData[i]["replaymodel"] == 'null') {
			aHtml += ' <div class="cs-reply"><!-- 客服回复 -->' + '<span><font style="font-weight:bold ">[客服回复  ]</font>：' + aData[i]["replaymodel"] + '</span>' + '</div>' + '</li>';
		}
	}
...
senseluxuryFed.detailFun.setcommentFun = function(infoText) {
	var text = infoText,
		_this = senseluxuryFed.detailFun,
		$subId = $('#pid').val(),
		refreshtoken = $("#hid_refreshtoken").val();
	domain = $('#domain').val();
	$.ajax({
...
$(function() {
	...
	var price_1 = 0,
		price_2 = 0,
		sumPrice = 0;
	sumPrice = parseInt($('#addPrice_3').val()) + parseInt($('#addPrice_4').val()) + parseInt($('#addPrice_5').val());
	var coupons = 0;
	if (sumPrice > 0) {
		$('.zj i').text(sumPrice);
	...
	var countCoupons = function(couponsSwitch) {
			var $openList = $(".coupons .list.open");
			var $active = $openList.find(".active");
			var price = 0;
			var checkRank = new Array();
			if (!couponsSwitch) $active.each(function(index, item) {
				var $item = $(item);
				price = price + (~~ ($item.find(".price").text()));
				checkRank.push($item.find("#hid_rank_id").val());
			});
			coupons = price;
			$(".coupons .coupons-switch span").text(coupons);
			$('.zj i').text(sumPrice - coupons);
			$('#sumPrice').val(sumPrice - coupons);
			$("#hid_rank_total").val(coupons);
			$("#hid_rank_checkid").val(checkRank.join(","));
		};
	$(".coupons .type-switch").on("click", null, function(event) {
		var $this = $(this);
		var $list = $this.parent().find("table");
		var $item = $this.parent();
		var flag = $item.hasClass("open");
		$(".coupons .list").removeClass("open");
		if (flag) {
	...
	$(".coupons .append td p").on("click", null, function(event) {
		var $this = $(this);
		var $item = $this.parent().parent();
		if ($(".append tr.active").length >= 10) {
			alert('最多可同时使用10张可叠加抵用券');
			return;
		}
		if ($(".cant-append tr").length > 0) {
			$(".cant-append tr").removeClass('active');
		}
	...
	$(".coupons .cant-append td p").on("click", null, function(event) {
		var $this = $(this);
		var $item = $this.parent().parent();
		$(".append tr").removeClass('active');
		$(".coupons .cant-append tr").removeClass("active");
		$item.addClass("active");
		countCoupons();
	});
	$('#submit_inquire').click(function() {
		if ($("#hid_rank_checkid").val()) {
			$.post('/rankisused', {
				rankcheckids: $("#hid_rank_checkid").val()
			}, function(d) {
				if (d.code !== 1) {
					alert(d.msg);
					return false;
	...
	$('.select-c-list').eq(0).find('li').click(function() {
		var liVal = $(this).text();
		$("#span-val").attr('data-val', parseInt($(this).attr('data-val') * 2));
		$("#span-val").html(parseInt($(this).attr('data-val') * 2));
		$(this).parents('.select-c-list').hide().siblings('span').attr('data-flag', $(this).attr('data-flag'));
		$(this).parents('.select-c-list').hide().siblings('span').attr('data-val', $(this).attr('data-val'));
		$(this).parents('.select-c-list').hide().siblings('span').attr('data-standard', $(this).attr('data-standard'));
...
(function($) {
	function tipPop(element) {
		this._tip = $('<span class="tip_box"><font><em></em><p></p></font></span>');
		$("body").append(this._tip);
		this._element = null;
		this._text = "";
		this.setElement(element);
	}
	tipPop.prototype.setElement = function(element) {
		this._element = $(element).parent() || null;
		this._text = this._element.attr("data-msg") || "";
		this._tip.find("p").text(this._text);
	};
	tipPop.prototype.show = function() {
		this._tip.css({
			"display": "block"
		});
	};
	tipPop.prototype.hide = function() {
		this._tip.css({
			"display": "none"
		});
	};
	tipPop.prototype.moveTo = function(top, left) {
		this._tip.css({
			"top": top,
			"left": left
		});
	};
	tipPop.prototype.getTip = function() {
		return this._tip;
	};
	$tip = new tipPop();
	$(".ssfw-table").find("td").each(function(index, item) {
		var $item = $(item);
		var msg = $item.attr("data-msg");
		if (msg) {
			var tip = $tip.getTip();
			$item.html("<span>" + $item.text() + "</span>");
			$item.find("span").mousemove(function(event) {
				$tip.setElement(this);
				var arrow = tip.find("em");
				var top = $(this).offset().top;
				var left = $(this).offset().left - tip.width() / 2 + ($(this).width() / 2);
				if (event.clientY + tip.height() + 11 + 10 > $(window).height()) {
					$tip.moveTo(top - tip.height() - 11, left);
					arrow.css({
						"top": tip.height() - 4,
						"background": "url(http://senseluxury.com/statics/web3/images/hint_bg_down.png) top center no-repeat"
					});
				} else {
					$tip.moveTo(top + 20 + 11, left);
					arrow.css({
						"top": -11,
						"background": "url(http://senseluxury.com/statics/web3/images/hint_bg.png) top center no-repeat"
					});
				}
				$tip.show();
			});
			$item.find("span").mouseleave(function(event) {
				$tip.hide();
			});
		}
	});
...
function datetime_to_unix_d(datetime) {
	var arr = datetime.split("/");
	var now = new Date(Date.UTC(arr[0], arr[1] - 1, arr[2]));
	return parseInt(now.getTime() / 86400000);
}
